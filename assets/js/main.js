...
(function initContactLinks() {
  const whatsappNumber = ['1', '404', '520', '7353'].join('');

  document.querySelectorAll('[data-whatsapp-link]').forEach(link => {
    const preset = link.getAttribute('data-whatsapp-text');
    const text = preset ? `?text=${encodeURIComponent(preset)}` : '';
    link.href = `https://wa.me/${whatsappNumber}${text}`;
    link.target = '_blank';
